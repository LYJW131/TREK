...
function controller(allowed = true): AmapProxyController {
  const rl = { check: vi.fn(() => allowed) } as unknown as RateLimitService;
  return new AmapProxyController(rl);
}
...
beforeEach(() => {
  process.env.AMAP_JS_SECURITY_CODE = SECRET;
...
describe('AmapProxyController', () => {
  it('AMAP-120: appends jscode server-side and never echoes it back', async () => {
    ...
    const res = response();
    await controller().forward(request('/v3/log/init', 'key=abc&callback=cb_1') as never, res as never);
  ...
  it('AMAP-121: custom map styles go to the other host Amap documents', async () => {
    ...
    await controller().forward(request('/v4/map/styles', 's=1') as never, response() as never);
  ...
  it('AMAP-122: an instance with no security code says so instead of forwarding without it', async () => {
    ...
    await expect(controller().forward(request('/v3/log/init') as never, response() as never))
  ...
  it('AMAP-123: only Amap\'s versioned API paths are forwarded', async () => {
    for (const path of ['/internal/secrets', '/v3/../../etc/passwd', '/', '/v3']) {
      await expect(controller().forward(request(path) as never, response() as never))
  ...
  it('AMAP-124: a third party\'s Set-Cookie does not land on this app\'s domain', async () => {
    ...
    const res = response();
    await controller().forward(request('/v3/log/init') as never, res as never);
  ...
  it('AMAP-125: the limiter guards a route that spends the operator\'s quota', async () => {
    await expect(controller(false).forward(request('/v3/log/init') as never, response() as never))
