...
const admin: TranslationStrings = {
  ...
  'admin.transitProvider.amap': '高德地图（Amap）',
  ...
  'admin.transitProvider.amapHint':
    '使用上方的高德密钥，覆盖中国大陆——这里 Transitous 没有任何数据。高德返回的是行程时长而非发车时刻，因此显示的时间是估算值。',
  'admin.transitProvider.amapNoKeyWarning':
    '已选择 Amap，但未配置 Amap 密钥——公共交通搜索仍在使用 Transitous。请在上方的 API 密钥中添加密钥。',
  'admin.transitProvider.amapPersonalKeyWarning':
    '只设置了你自己的 Amap 密钥，因此其他成员的搜索仍会回退到 Transitous。请以管理员身份在上方保存密钥，使其对整个实例生效。',
